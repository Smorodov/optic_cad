...
Interpolate1d<X>::Interpolate1d ()
    : _update (&Interpolate1d::update_linear), _interpolate (_update), _poly ()
{
}

template <class X> Interpolate1d<X>::~Interpolate1d () {}

template <class X>
template <cubic_2nd_deriv_init_e de>
void
Interpolate1d<X>::compute_cubic_2nd_deriv (unsigned int n, double dd[],
    ...
    case Cubic2ndDerivFirst:
      {
        ...
        dd[n - 1]
            = dn - (X::get_y_value (n - 1) - X::get_y_value (n - 2)) / xn;
        eq[_ (0, 1)] = x0 / 3.0;
        eq[_ (1, 0)] = x0 / 6.0;
        eq[_ (n - 2, 2)] = xn / 6.0;
        eq[_ (n - 1, 1)] = xn / 3.0;
  ...
  for (i = 1; i < (int)n - 1; i++)
    {
      ...
      dd[i] = (X::get_y_value (i + 1) - X::get_y_value (i))
                  / X::get_x_interval (i)
              - (X::get_y_value (i) - X::get_y_value (i - 1))
                    / X::get_x_interval (i - 1);
    }
...
template <class X>
inline void
Interpolate1d<X>::set_linear_poly (struct poly_s &p, double p1x, double p1y,
...
template <class X>
inline void
Interpolate1d<X>::set_quadratic_poly (struct poly_s &p, double p1x, double p1y,
...
template <class X>
inline void
Interpolate1d<X>::set_cubic_poly (struct poly_s &p, double p1x, double p1y,
...
template <class X>
inline void
Interpolate1d<X>::set_cubic_poly2 (struct poly_s &p, double p1x, double p1y,
...
template <class X>
inline void
Interpolate1d<X>::set_quadratic_poly (struct poly_s &p, double px, double py,
                                      double d, double dd)
{
  p.a = 0;
  p.b = dd / 2.0;
...
template <class X>
void
Interpolate1d<X>::set_interpolation (Interpolation i)
{
  switch (i)
    {
    case Nearest:
...
template <class X>
double
Interpolate1d<X>::interpolate_nearest (unsigned int d, double x) const
{
  switch (d)
    {
    case (0):
...
template <class X>
double
Interpolate1d<X>::update_nearest (unsigned int d, double x) const
{
...
template <class X>
double
Interpolate1d<X>::interpolate_linear (unsigned int d, double x) const
{
  ...
  switch (d)
    ...
    case (0):
      {
        double mu
            = (x - X::get_x_value (di - 1)) / (X::get_x_interval (di - 1));
    ...
    case (1):
      {
        return (X::get_y_value (di) - X::get_y_value (di - 1))
               / (X::get_x_interval (di - 1));
      }
...
template <class X>
double
Interpolate1d<X>::update_linear (unsigned int d, double x) const
{
  Interpolate1d *this_ = const_cast<Interpolate1d *> (this);
...
template <class X>
double
Interpolate1d<X>::interpolate_quadratic (unsigned int d, double x) const
{
...
template <class X>
double
Interpolate1d<X>::update_quadratic (unsigned int d, double x) const
{
  Interpolate1d *this_ = const_cast<Interpolate1d *> (this);
  ...
  for (i = 1; i < X::get_count () - 1; i++)
    {
      ...
      set_quadratic_poly (poly[i], p1x, p1y, X::get_x_value (i),
                          X::get_y_value (i), p3x, p3y);
    }

  set_linear_poly (poly[i], X::get_x_value (i - 1), X::get_y_value (i - 1),
...
template <class X>
double
Interpolate1d<X>::interpolate_cubic (unsigned int d, double x) const
{
...
template <class X>
double
Interpolate1d<X>::update_cubic_simple (unsigned int d, double x) const
{
  Interpolate1d *this_ = const_cast<Interpolate1d *> (this);
  std::vector<struct poly_s> &poly = this_->_poly;
...
template <class X>
double
Interpolate1d<X>::update_cubic (unsigned int d, double x) const
{
  Interpolate1d *this_ = const_cast<Interpolate1d *> (this);
  std::vector<struct poly_s> &poly = this_->_poly;
  ...
  double d0
      = (X::get_y_value (1) - X::get_y_value (0)) / X::get_x_interval (0);
  double dn = (X::get_y_value (n - 1) - X::get_y_value (n - 2))
              / X::get_x_interval (n - 2);
  // double dd[n];
  ...
  for (unsigned int i = 1; i < n; i++)
    set_cubic_poly2 (poly[i], X::get_x_value (i - 1), X::get_y_value (i - 1),
                     X::get_x_value (i), X::get_y_value (i), dd[i - 1], dd[i]);

  set_linear_poly (poly[n], X::get_x_value (n - 1), X::get_y_value (n - 1),
                   dn);
...
template <class X>
double
Interpolate1d<X>::update_cubic2 (unsigned int d, double x) const
{
  Interpolate1d *this_ = const_cast<Interpolate1d *> (this);
  std::vector<struct poly_s> &poly = this_->_poly;
  ...
  set_quadratic_poly (poly[0], X::get_x_value (0), X::get_y_value (0), d0,
                      dd[0]);
  ...
  set_quadratic_poly (poly[n], X::get_x_value (n - 1), X::get_y_value (n - 1),
                      dn, dd[n - 1]);
...
template <class X>
double
Interpolate1d<X>::update_cubic_deriv_init (unsigned int d, double x) const
{
  Interpolate1d *this_ = const_cast<Interpolate1d *> (this);
  std::vector<struct poly_s> &poly = this_->_poly;
...
template <class X>
double
Interpolate1d<X>::update_cubic2_deriv_init (unsigned int d, double x) const
{
  Interpolate1d *this_ = const_cast<Interpolate1d *> (this);
  std::vector<struct poly_s> &poly = this_->_poly;
...
template <class X>
double
Interpolate1d<X>::update_cubic2_deriv (unsigned int d, double x) const
{
  Interpolate1d *this_ = const_cast<Interpolate1d *> (this);
  std::vector<struct poly_s> &poly = this_->_poly;
  ...
  double dd0
      = (X::get_d_value (1) - X::get_d_value (0)) / X::get_x_interval (0);

  set_quadratic_poly (poly[0], X::get_x_value (0), X::get_y_value (0),
                      X::get_d_value (0), dd0);

  for (unsigned int i = 1; i < n; i++)
    set_cubic_poly (poly[i], X::get_x_value (i - 1), X::get_y_value (i - 1),
                    X::get_x_value (i), X::get_y_value (i),
                    X::get_d_value (i - 1), X::get_d_value (i));

  double ddn = (X::get_d_value (n - 1) - X::get_d_value (n - 2))
               / X::get_x_interval (n - 2);
...
template <class X>
double
Interpolate1d<X>::update_cubic_deriv (unsigned int d, double x) const
{
  Interpolate1d *this_ = const_cast<Interpolate1d *> (this);
  std::vector<struct poly_s> &poly = this_->_poly;
